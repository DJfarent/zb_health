...
This mod removes the complex medical UI (blood, organs, pulse, oxygen, etc.) and focuses purely on **limb condition**.
...
Most Z-City HUD mods include many systems:
...
This mod intentionally removes those and instead focuses on **one core mechanic: limb damage**.
...
Each limb has four possible states:
...
The HUD will automatically appear when joining the server.
...
The HUD reads damage values from:
...
This HUD is designed to be **very lightweight**.
...
Possible future features while keeping the HUD simple:
